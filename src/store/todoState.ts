import {generateRandomId} from '@/utils';
import {atom, selector} from 'recoil';

const DUMMY_TODO = [
  {
    id: generateRandomId(),
    title: '나의 미래 준비, 어떻게 시작할까요?',
    field: {key: 'ECONOMY', text: '경제'},
    tags: ['경제'],
    subTodoList: [
      {id: generateRandomId(), title: '노후 예상 비용 계산하기', isDone: true},
      {id: generateRandomId(), title: '월 저축액 계획하기', isDone: false},
      {id: generateRandomId(), title: '월 저축액 계획하기', isDone: false},
      {id: generateRandomId(), title: '월 저축액 계획하기', isDone: false},
      {id: generateRandomId(), title: '월 저축액 계획하기', isDone: false},
      {id: generateRandomId(), title: '월 저축액 계획하기', isDone: false},
      {id: generateRandomId(), title: '월 저축액 계획하기', isDone: false},
    ],
    createdDate: '2024-01-01',
    completedDate: null,
    isDone: false,
  },
  {
    id: generateRandomId(),
    title: '사용자가 입력한 할 일 제목 1',
    field: {key: 'HEALTH', text: '건강'},
    tags: ['나의 할 일', '건강'],
    subTodoList: [
      {id: generateRandomId(), title: '2024 건강검진 하기', isDone: false},
    ],
    createdDate: '2024-01-10',
    completedDate: null,
    isDone: false,
  },
  {
    id: generateRandomId(),
    title: '사용자가 입력한 할 일 제목 2',
    field: {key: 'SELFDEV', text: '자기계발'},
    tags: ['완료', '나의 할 일', '자기계발'],
    subTodoList: [
      {id: generateRandomId(), title: '자격증 취득', isDone: true},
      {id: generateRandomId(), title: '토익 900점 이상', isDone: true},
    ],
    createdDate: '2024-01-01',
    completedDate: '2024-01-20',
    isDone: true,
  },
  {
    id: generateRandomId(),
    title: '사용자가 입력한 할 일 제목 3',
    field: {key: 'SELFDEV', text: '자기계발'},
    tags: ['나의 할 일', '자기계발'],
    subTodoList: [],
    createdDate: '2024-01-31',
    completedDate: null,
    isDone: false,
  },
];

// atom
const todoListState = atom({
  key: 'todoListState',
  // default: DUMMY_TODO,
  default: [],
});

const todoListFilterState = atom({
  key: 'TodoListFilter',
  default: '진행 중',
});

const filteredTodoListState = selector({
  key: 'FilteredTodoListState',
  get: ({get}) => {
    const filter = get(todoListFilterState);
    const list = get(todoListState);

    if (filter === '진행 중') {
      return list.filter(item => !item?.hasDone);
    }

    if (filter === '완료') {
      return list.filter(item => item?.hasDone);
    }
  },
});

export {todoListState, todoListFilterState, filteredTodoListState};
